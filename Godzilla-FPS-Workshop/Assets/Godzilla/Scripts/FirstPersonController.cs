...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		[Header("Godzilla_Workshop")]
		[Tooltip("Helper functions and dependencies for Godzilla Workshop")]
		...
		private void Awake()
		{
			...
			attackControl = new Attack();
			attackControl.Enable();
			//attackControl["attack"].performed += handleAttack;
		...
		private void FixedUpdate()
		{
			...
			if (cast)
			{
				...
				{
					if (hit.collider.gameObject.tag == "MechaGodzilla")
					{
						GlueScript.damageMechaGodzillaEvent(0.125f);
					}
		...
		private void Move()
		{
			...
			//Speed is intact and correct at this point. 01/13/2024 Bookmark
			Animator animator = Animator_Obj.GetComponent<Animator>();

			animator.SetFloat("Velocity", _speed / MoveSpeed);
		...
		void handleAttack(InputAction.CallbackContext context)
		{
			...
			// Spawn prefab with collider
			float x = attackColliderSpawnPoint.transform.position.x;
			float y = attackColliderSpawnPoint.transform.position.y;
			float z = attackColliderSpawnPoint.transform.position.z;
			Instantiate(attackCollider, new Vector3(x, y, z), Quaternion.identity);
			// Check for collision
